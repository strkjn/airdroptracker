...
import 'package:airdrop_flow/main.dart'; // Import main.dart untuk akses notificationService
import 'package:flutter/material.dart';
import 'package:flutter_riverpod/flutter_riverpod.dart';

// Ubah menjadi ConsumerStatefulWidget agar kita bisa menggunakan initState
...
class _MainScaffoldState extends ConsumerState<MainScaffold> {
  ...
  void initState() {
    super.initState();
    // Jalankan logika penjadwalan notifikasi setelah widget pertama kali dibuat
    _scheduleDailyNotification();
  }

  void _scheduleDailyNotification() async {
    // Kita perlu sedikit menunda untuk memastikan provider siap dibaca
    await Future.delayed(const Duration(seconds: 2));

    // Baca data tugas hari ini dari provider
    final tasksAsync = ref.read(todaysTasksProvider);
    
    tasksAsync.whenData((tasks) {
      if (tasks.isNotEmpty) {
        final taskCount = tasks.length;
        // Panggil service notifikasi yang sudah kita buat di main.dart
        notificationService.scheduleDailySummaryNotification(
          hour: 7, // Jadwalkan notifikasi untuk jam 8...
          minute: 0, // ...pagi setiap hari
      ...
      bottomNavigationBar: BottomAppBar(
        ...
        child: Row(
          ...
          children: <Widget>[
            ...
            _buildNavItem(icon: Icons.list_alt_outlined, label: 'Proyek', index: 1),
            const SizedBox(width: 40),
            _buildNavItem(icon: Icons.explore_outlined, label: 'Discover', index: 2),
            _buildNavItem(icon: Icons.settings_outlined, label: 'Pengaturan', index: 3),
          ],
        ),
      ),
    );
